...
## GenAI policy reminder (Week 1) {.smaller}

- ✅ Allowed: asking **clarifying questions** (syntax, errors, concepts)
- ❌ Not allowed: generating code, solutions, reports, or summaries

When stuck:

1. Reduce the problem (smallest reproducible case)
2. Read the error message *slowly*
3. Use `help()` / docs
4. Ask the instructor/TA with the exact error + what you tried

---

## Warm-up (5 minutes)

Run your Day 2 project.
...
## Modules + packages + built-in modules {.r-fit-text}
...
## OOP essentials (only what you need) {.r-fit-text}
...
- Recognize and use **inheritance** when appropriate
...
A property lets you **validate** data when it changes.
...
Why not store age directly as `self.age = value`?

A) It’s slower
B) You can’t validate changes
C) Python will crash
...
- `repr()` makes debugging easier
- `==` becomes meaningful (useful in tests)

---

## Mini-exercise: use the `Person` class (6 minutes)
...
**Checkpoint:** you get an assertion error.
...
- `first_name` and `last_name` come from properties
- Assigning `300` triggers the setter and fails

::: callout-tip
This is a good mental model: “assignment can run logic”.
:::
...
::: aside
For this week: you can ignore multiple inheritance.
:::
...
- Python cares about “does it support `.count()`?”
- not “what exact class is it?”

---

## OOP in our project (two options)

**Option A (fine):** use dicts

```python
{"name": "age", "type": "number", "missing": 2, "mean": 24.3}
```

**Option B (cleaner):** use a class

```python
ColumnProfile(name="age", inferred_type="number", missing=2, ...)
```

Today we’ll implement a small class to practice.
...
A) A nested dict with 10 keys
B) A small object with `__repr__`
...
- Encapsulation: properties enforce rules
- Inheritance: reuse behavior (use carefully)
- Polymorphism: same interface across different types
- A light OOP model can make your report cleaner
...
## Typer CLI from type hints {.r-fit-text}
...
**Deliverable by 9:00pm:** CLI works on `data/sample.csv`.
...
```python {code-line-numbers="1-3|4-7|8-10|11-17|18-22|5,23|2,24|"}
...
- `timing_ms` exists in JSON
- Markdown table lists all columns
