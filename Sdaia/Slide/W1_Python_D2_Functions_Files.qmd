...
## GenAI policy reminder (Week 1)

- ✅ Allowed: asking **clarifying questions** (syntax, errors, concepts)
- ❌ Not allowed: generating code, solutions, reports, or summaries

If you are stuck:
1. Re-read your own code
2. Use docs (`help()`, official docs)
3. Ask a human (TA / instructor)

::: aside
This week is about *building your own skills*, not outsourcing them.
:::
...
## If imports fail: fix your `src/` imports (for now) {.smaller}

If you see `ModuleNotFoundError: No module named 'csv_profiler'`:

**Option A (recommended today): run with `PYTHONPATH=src`**
...
::: callout-tip
We’ll formalize packaging later. Today we use this simple approach to keep moving.
:::
...
- Read a CSV into `rows: list[dict[str, str]]`
...
## Functions: your unit of thinking {.r-fit-text}
...
- the return type after `->` is a **hint** (not enforced automatically)
...
def double(**values: float) -> dict[str, float]:
    return {k: v * 2 for k, v in values.items()}
...
## Strings → Markdown reports {.r-fit-text}
...
## Files + `pathlib` + formats {.r-fit-text}
...
## Hands-on: CSV Profiler (Part 2) {.r-fit-text}
...
In `src/csv_profiler/profile.py`, add:
...
In `src/csv_profiler/render.py`, update `write_markdown(report, path)` to include:
